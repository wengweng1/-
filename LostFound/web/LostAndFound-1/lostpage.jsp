    ...
    <div class="doc-container" id="doc-container">
        ...
        <div class="container-fixed">
		    ...
		    <div class="col-content">
		        ...
		        <div class="inner">
				<c:forEach var="lost" items="${sessionScope.lostGoodsList}">
		            <article class="article-list bloglist" id="LAY_bloglist" >
						...
						<section class="article-item zoomIn article">
							...
							<h5 class="title" >
								<span >
									<a class="layui-btn layui-btn-radius layui-btn-sm layui-btn-danger layer-demolist" href="javascript:;" id="test4${obj.id}" onmouseover="show('test4${obj.id}')" data-tooltip="正在寻找">失物招领</a>
								</span>

							</h5>
							<div class="time">
								<span class="day">${lost.i_time}</span>
							</div>
							<div class="content">
							
								<a href="detail.jsp?postId=${lost.i_id}" target="_blank" class="cover img-light">
									<img src="<%=basePath%>/${lost.i_img}" alt="">
								</a>
								<p class="layui-btn layui-btn-primary layui-border-red">地址信息：${lost.i_place}</p>
								<br>
								<p  class="btn">&nbsp;&nbsp;&nbsp; 特征展示：${lost.i_sort}</p>
								
							</div>
							<div class="read-more">
								<a href="<%=request.getContextPath()%>/LostServlet?op=getmessage&postId=${lost.i_id}" class="fc-black f-fwb" target="_blank">查看详情</a>
							</div>
							<aside class="f-oh footer">
								<div class="f-fl tags">
									<span class="fa fa-tags fs-16"></span>
									<a class="tag">${lost.i_name}</a>
								</div>
								<div class="f-fl tags">
									<i class="layui-icon fa fa-tags fs-16">&#xe60e;</i>
									<a class="tag">${lost.i_time}</a>
								</div>
								<div class="f-fr">
									<span class="ml20">               
										<i class="fa fa-comments fs-16"></i>               
										<a href="javascript:void(0)" class="num fc-grey">${lost.i_telephone}</a>
									</span>
								</div>
							</aside>
						</section>
...
<%--				        	<li data-index="0" class="slider"></li>--%>
								<li class="style"><a href="<%=request.getContextPath()%>/LostServlet?op=getbyid&id=${type.g_id}">${type.g_name}</a></li>
							</c:forEach>
...
<script>
	function selectPost(){
		...
		$.ajax({
			url:"<%=request.getContextPath()%>/LostServlet?op=searchtxt&name="+searchtxt,
			type:"post",
